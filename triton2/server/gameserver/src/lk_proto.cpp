				        ...
				        void (*pformat)(CMaxString&, const char*, const char*, const char*))
{
	...
	{
		LOG_ERROR("default", "Index must be -1 for non-repeated fields:%s\n", field->name().c_str());
		return;
	}
	...
	for (; child != NULL; child = child->NextSibling())
	{
		switch(child->Type())
		{
			case TiXmlNode::ELEMENT:
			{
				...
				if (pFieldDef == NULL)
				{
					LOG_ERROR("default", "Message (%s) has no field (%s), pls check protocol\n",
				...
				if ( pFieldDef->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE)
				{
					...
					if ( pSubMessage == NULL )
					{
						LOG_ERROR("default", "Can not add message for field (%s) of message (%s)\n", 
			...
			case TiXmlNode::TEXT:
			{
				...
				switch(pFieldDef->cpp_type())
				{
					...
					case FieldDescriptor::CPPTYPE_ENUM:
					{
						...
						if ( pEnumValue == NULL)
						{
							LOG_ERROR("default", "Can not fine enum text(%s) of message(%s)\n", 
					...
					default:
					{
						LOG_ERROR("default", "The Msg(%s) don't support field(%s) type(%d) value(%s)", 
