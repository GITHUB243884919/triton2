...
int CMD5::compute(const void *pInput, int32_t nInputLength, void *pDigest)
{
	int nResult  = -1;

	const uint8_t *pucInput = (const uint8_t *)pInput;
	uint8_t       *pucDigest = (uint8_t *)pDigest;

	struct MD5Context s;

	CMD5::MD5Init(&s);
	CMD5::MD5Update(&s, pucInput, nInputLength);
	CMD5::MD5Final(pucDigest, &s);

	nResult = 0;
	return nResult;
}

void CMD5::MD5Init(MD5Context *s)
{
...
void CMD5::MD5Update(MD5Context *s, const uint8_t *p, uint32_t len)
{
	uint8_t *q    = (uint8_t *) p;
	uint32_t lenw = len;
	...
	if (s->blkused + len < ( unsigned int )BLKSIZE) {
		/*
		 ...
		 */
		while (s->blkused + len >= ( unsigned int )BLKSIZE) {
...
void CMD5::MD5Final(uint8_t output[], MD5Context *s)
{
	int32_t  i;
	uint32_t pad;
	uint8_t  c[64];
...
void CMD5::MD5_Core_Init(MD5_Core_State *s)
{
	s->h[0] = 0x67452301;
	s->h[1] = 0xefcdab89;
	s->h[2] = 0x98badcfe;
...
void CMD5::MD5_Block(MD5_Core_State *s, uint32_t *block)
{
	uint32_t a, b, c, d;
	a = s->h[0];
	b = s->h[1];
