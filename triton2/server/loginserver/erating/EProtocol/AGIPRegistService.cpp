...
uint32_t AGIPRegistService::getGameID(void)
{
    return ntohl(m_data->un_Game_ID);
}

void AGIPRegistService::setGameID(uint32_t unGameID)
{
    m_data->un_Game_ID = htonl(unGameID);
}

uint32_t AGIPRegistService::getGatewayID(void)
{
    return ntohl(m_data->un_Gateway_ID);
}

void AGIPRegistService::setGatewayID(uint32_t unGatewayID)
{
