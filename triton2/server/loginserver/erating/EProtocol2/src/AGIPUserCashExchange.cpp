...
{
    return ntohll(*((uint64_t*)(&(((PSAGIPUserCashExchangeRes)m_pucPDU)->un_detail_id_h))), nFlag);
}

void   AGIPUserCashExchangeRes::setDetailID(uint64_t ullDetailID,int32_t nFlag)
{
    *((uint64_t*)(&(((PSAGIPUserCashExchangeRes)m_pucPDU)->un_detail_id_h))) = htonll(ullDetailID, nFlag);
}
