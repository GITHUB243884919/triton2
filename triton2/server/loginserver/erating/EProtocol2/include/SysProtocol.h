...
#define AGIP_CURRENT_VERSION            0x20
...
Engine_Export const char* stringTime(time_t tm, char []);
...
public:
    ...
    int32_t getGameID(uint32_t* punGameID) const;

    void setGatewayID(uint32_t unGatewayID);
    uint32_t getGatewayID(void) const;
    int32_t getGatewayID(uint32_t* punGatewayID) const;

    /* Copy sequence ID from pSysProtocol directly. */
    int cloneSequenceID(const SysProtocol *pSysProtocol);
    /* Copy game ID from pSysProtocol directly. */
    int cloneGameID(const SysProtocol *pSysProtocol);
    /* Copy gateway ID from pSysProtocol directly. */
    int cloneGatewayID(const SysProtocol *pSysProtocol);
    /* Compare binary stream between this and pSysProtocol. */
    int compare(const SysProtocol *pSysProtocol, int *pnBytesEqual);

    // Copy sequence_id, game_id, gateway_id from pSysProtocol to this directly
    int clone(const SysProtocol *pSysProtocol);

    char* getLogBuf(void);

    size_t getLogBufSize(void);
...
private:
    ...
    char            m_szLogBuf[MAX_LOG_BUFFER_SIZE + 1];
    size_t          m_unLogBufLen;
};
