...
public:
	...
	int Send(CMessage* pMsg, CLoginPlayer* pPlayers);
	int Send(CMessage* pMsg, uint32 vCommHandle, uint32 vSockTime, int vClientAddr, unsigned int vClientPort);
	...
	int EncodeNetMsg( CMessage* pMsg, BYTE* pCodeBuff, int& vLen, CLoginPlayer* ppPlayer);
	int EncodeNetMsg( CMessage* pMsg, BYTE* pCodeBuff, int& vLen, uint32 vCommnHandle, uint32 vSockTime );
