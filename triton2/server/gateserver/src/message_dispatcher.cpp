...
int CMessageDispatcher::ProcessMessage( CMessage* vMsg )
{
	...
	switch( vMsg->msghead().srcfe() )
	{
		...
		{
			LOG_DEBUG( "default", "---- Recv Proxy Msg[ %s ][id: 0x%04x / %d] ----", 
					pDescriptor->name().c_str(), vMsg->msghead().messageid(), vMsg->msghead().messageid() );
			LOG_DEBUG( "default", "[%s]", ((Message*)vMsg->msgpara())->ShortDebugString().c_str() );
			LOG_DEBUG( "default", "\n" );
			CGateLogic::GetSingletonPtr()->OnProxyMessage( vMsg );
			break;
		}
		*/
		case FE_CLIENT:
		{
			...
			LOG_DEBUG( "default", "[%s]", ((Message*)vMsg->msgpara())->ShortDebugString().c_str() );
			LOG_DEBUG( "default", "\n" );
			GateServer::CGateLogic::GetSingletonPtr()->OnClientMessage( vMsg );
			break;
		}
		default:
		{
