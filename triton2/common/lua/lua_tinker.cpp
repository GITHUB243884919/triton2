...
void lua_tinker::print_error(lua_State *L, const char* fmt, ...)
{
	...
	{
#ifdef LINUX
		FILE* tpFile = fopen( "lua_error.out", "a+" );
		if( tpFile != NULL )
		{	
			time_t tTime = time( NULL );
			struct tm* tpTime = localtime( &tTime );
			if( tpTime != NULL )
			{
				fprintf( tpFile, "[ %04d-%02d-%02d %02d:%02d:%02d ]", 1900 + tpTime->tm_year, 1 + tpTime->tm_mon,
					tpTime->tm_mday, tpTime->tm_hour, tpTime->tm_min, tpTime->tm_sec );
			}
			fprintf( tpFile, text );
			fprintf( tpFile, "\n" );
			fclose( tpFile );
